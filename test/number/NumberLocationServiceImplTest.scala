...
class NumberLocationServiceImplTest extends Specification {
  ...
  "Spaces" should {
    "be ignored" in {
      " +34  10 987654 ".toLocation must beEqualTo(Success((Some("Madrid"), "987654", "+3410987654", Seq("Spain"))))
    }
  }

  "A local number" should {
    "have the local city and country" in {
      "999888".toLocation must beEqualTo(Success((Some("Basingstoke"), "999888", "+441256999888", Seq("United Kingdom"))))
    }
  }

  "A national number" should {
    "have the city and country" in {
      "01483123456".toLocation must beEqualTo(Success((Some("Guildford"), "123456", "+441483123456", Seq("United Kingdom"))))
    }
  }

  "A local non-geographic number" should {
    "have no city but all the possible countries" in {
      "0800999888".toLocation must beEqualTo(
  ...
  "An international number with a 00 prefix" should {
    "have a city and one country" in {
      "003410987654".toLocation must beEqualTo(Success((Some("Madrid"), "987654", "+3410987654", Seq("Spain"))))
    }
  }

  "An international number with a + prefix" should {
    "have a city and one country" in {
      "+3410987654".toLocation must beEqualTo(Success((Some("Madrid"), "987654", "+3410987654", Seq("Spain"))))
    }
  }

  "An international non-geographic number with a 00 prefix" should {
    "have a country but no city" in {
      "003490987654".toLocation must beEqualTo(Success((None, "90987654", "+3490987654", Seq("Spain"))))
    }
  }

  "An international non-geographic number with a + prefix" should {
    "have a country but no city" in {
      "+3490987654".toLocation must beEqualTo(Success((None, "90987654", "+3490987654", Seq("Spain"))))
    }
  }
