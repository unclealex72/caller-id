...
abstract class AbstractDustTest(dustTemplateName: String) extends FunSuite with BeforeAndAfter {
  ...
  before {
    val dustReader = new InputStreamReader(
      getClass.getClassLoader.getResourceAsStream("resources/public/js/dust-full-1.2.2.js"), "UTF-8")
    ...
    val templateSource = Source.fromInputStream(
      getClass.getClassLoader.getResourceAsStream(s"resources/template/${dustTemplateName}.dust")).mkString
